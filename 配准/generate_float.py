...
if __name__ == '__main__':
    generate_float_img(idx=3)

    # source = cv2.imread("float2.jpg" ,cv2.IMREAD_GRAYSCALE)   #浮动图像
    # target = cv2.imread("base2.jpg" ,cv2.IMREAD_GRAYSCALE) #基准图像
    # target = cv2.resize(target, source.shape[::-1])
    # diff = (np.int32(source)-np.int32(target))+130
    # # diff = np.int8(diff)
